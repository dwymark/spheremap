// Gnomonic - Snyder p. 165-166, eqs (22-4),(22-5),(22-6) forward;
// (20-14)-(20-19) inverse with (22-16) for c. Sphere.
// All great circles are straight lines; less than a hemisphere visible.

import { PI, HALF_PI, wrapDelta } from './base.js';

const R_GNOMONIC = 3.5;

export const gnomonic = {
  id: 'gnomonic',
  name: 'Gnomonic',
  property: 'compromise',
  family: 'azimuthal',
  description: 'Perspective from the sphere centre. Every great circle is a straight line.',
  snyder: 'Snyder p.165-166, eqs (22-4)-(22-6),(20-14)-(20-19),(22-16)',
  ...
  bounds() { return { xmin: -R_GNOMONIC, xmax: R_GNOMONIC, ymin: -R_GNOMONIC, ymax: R_GNOMONIC }; },
  ...
  forward(phi, lambda, p) {
    ...
    if (cosc <= 1e-6) return [0, 0, 0];
    return [
      Math.cos(phi) * Math.sin(dl) / cosc,
      (Math.cos(p.phi1)*Math.sin(phi) - Math.sin(p.phi1)*Math.cos(phi)*Math.cos(dl)) / cosc,
      1,
    ];
  },
